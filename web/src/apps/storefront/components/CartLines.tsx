 ...
 * variant and chosen add-ons. `compact` (sheet, 20rem panel) stacks the full
 * name above the controls so it stays legible in a narrow column; the default
 * lays everything out on one row.
 */
  ...
  return (
    <ul className="divide-y divide-cream-200">
      {lines.map((l) => {
        ...
        const label = lineLabel(l.name, variantsText(l.variants), l.addons.map((a) => a.name).join(', ') || undefined);
        const stepper = (
          <Stepper
            qty={l.qty}
            size={compact ? 'sm' : 'md'}
          ...
          return (
            <li key={l.key} className="py-4">
              ...
              <div className="mt-2 flex items-center gap-3">
                {stepper}
                <span className="flex-1 text-right text-sm font-semibold text-brand-900">
                  {rupees(unit * l.qty)}
                </span>
                {remove}
              </div>
