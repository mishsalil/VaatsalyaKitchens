-- install_fresh.sql — ONE FILE, EMPTY DATABASE.
--
-- ============================================================================
-- WHAT THIS IS
-- ============================================================================
-- Everything needed to stand the app up on a brand-new, EMPTY database:
-- schema.sql (the base tables and the starter menu) followed by every
-- migration 001-012. Run it once, on a database with nothing in it.
--
-- THIS IS NOT THE UPGRADE PATH. To bring an EXISTING database up to date
-- without losing its data, run database/migrate_production.sql instead — that
-- one is guarded and safe to re-run, and it keeps every customer, order and
-- menu edit. This file is only for starting from nothing.
--
-- ============================================================================
-- IT REFUSES TO RUN ON A DATABASE THAT IS NOT EMPTY
-- ============================================================================
-- The guard below aborts with an error naming a table that does not exist:
--   Table "REFUSING_TO_RUN__database_is_not_empty" doesn't exist
-- That message IS the refusal. Nothing has been changed at that point. It is a
-- deliberately blunt mechanism because plain SQL has no way to stop politely.
--
-- ============================================================================
-- AFTER RUNNING IT
-- ============================================================================
-- 1. CHANGE THE ADMIN PASSWORD IMMEDIATELY. schema.sql seeds a single user
--    "admin" whose password hash is committed to this repository, so its
--    password is not a secret from anyone who can read the code.
-- 2. The menu is the small starter set from schema.sql, not any menu you had
--    before. Kitchen hours seed to 08:00-23:59 every day.
-- 3. Every row of the verification table printed at the end must read OK.
--
-- ============================================================================
-- DERIVED FILE — DO NOT EDIT BY HAND
-- ============================================================================
-- It is schema.sql + database/migrate_production.sql, in that order. Change
-- those and regenerate, or the three will disagree.
-- scripts/verify-cumulative-migration.php checks this file still matches the
-- numbered migrations.
-- ============================================================================


-- ============================================================================
-- GUARD — abort unless the database is empty.
-- ============================================================================
SET @existing := (SELECT COUNT(*) FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE());
SET @s := IF(@existing > 0,
  "SELECT * FROM `REFUSING_TO_RUN__database_is_not_empty`",
  "DO 0");
PREPARE st FROM @s; EXECUTE st; DEALLOCATE PREPARE st;


-- ============================================================================
-- PART 1 of 2 — schema.sql (base tables + starter menu)
-- ============================================================================
...
-- ============================================================================
-- PART 2 of 2 — every migration 001-012 (from database/migrate_production.sql)
-- ============================================================================
