...
the share of sales the kitchen is willing to give away — becomes three
codes, generated by `discount_plan()` in `includes/discounts.php`. Given
budget B (a percent) and the average pre-tax order A (delivered orders,
last 30 days; ₹400 until there are ten of them):

- **WELCOME** — 1.5×B%, first order only, capped at ₹100.
- **VK\<B\>** (e.g. `VK10`) — B%, minimum order ≈ the average order, capped
  at ₹150.
- **FEAST** — (B+5)%, minimum order ≈ 2× the average order, capped at ₹300.

No code's percentage ever exceeds **24%** — the same ceiling the counter
enforces when a code and a manual discount are applied together on the
same order.

Saving the budget (or clicking Regenerate) reruns `discount_plan()` and
writes it over the three codes: any code no longer in the plan is switched
off, not deleted, so its usage history survives. Renaming a code changes
its text but keeps its id — since usage is counted by matching
`orders.discount_code` against the current text, a rename restarts the
usage count shown in Admin → Discounts. Setting the budget to 0 turns the
feature off.
