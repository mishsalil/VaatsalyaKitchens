...
function route($method, $action, $parts): void
{
    ...
    if ($action === 'index' && $method === 'GET') {
        ...
        foreach ($items as &$it) {
            ...
            /* Keyed by SLOT, not a plain list, so the editor knows which of the
               three positions are filled — a list would lose that when slot 2
               is empty but slot 3 is not. */
            $it['photos'] = (object)($photos[$iid] ?? []);
        }
        unset($it);
    ...
    /* --- dish photos (see includes/dish_photos.php) ----------------------------
       Up to three per item, stored as files. The slot comes from the query
       string rather than the path so the id stays where every other item route
       puts it, in $parts[3]. Slot defaults to 1, which is the original
       single-photo behaviour. */
    if ($action === 'upload_image') {
        $id = (int)($parts[3] ?? 0);
        if (!item_exists($id)) {
            Response::error('Item not found.', 404);
        }
        $slot = (int)($_POST['slot'] ?? $_GET['slot'] ?? 1);
        Response::json(['url' => save_dish_image($id, $slot), 'slot' => $slot]);
    }
    ...
    if ($action === 'delete_image') {
        ...
        // No slot means every photo for the item.
        $slotRaw = $_POST['slot'] ?? $_GET['slot'] ?? null;
        delete_dish_image($id, $slotRaw === null ? null : (int)$slotRaw);
