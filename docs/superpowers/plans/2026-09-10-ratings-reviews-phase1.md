...
try {
    ...
    foreach ($candidates as $order) {
        ...
        try {
            ...
            /* push_send_to_customer returns a POSITIONAL [sent, failed] — there is
               no 'sent' key. Reading one would be silently 0 forever: every
               delivered push recorded as a failure, every order going quiet after
               three attempts. Destructure, matching the three call sites in
               api/routes/admin/orders.php. */
